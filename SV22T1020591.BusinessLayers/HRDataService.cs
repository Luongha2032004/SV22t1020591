...
namespace SV22T1020591.BusinessLayers
{
    ...
    public static class HRDataService
    {
        ...
        /// <summary>
        /// Lấy RoleNames (chuỗi, có thể nhiều role ngăn cách bởi dấu phẩy) từ bảng Employees.
        /// Thực hiện trực tiếp SQL bằng Dapper để tránh phụ thuộc vào nhiều phiên bản repository.
        /// </summary>
        public static async Task<string?> GetEmployeeRoleNamesAsync(int employeeID)
        {
            using var connection = new SqlConnection(Configuration.ConnectionString);
            const string sql = @"SELECT RoleNames FROM Employees WHERE EmployeeID = @EmployeeID";
            return await connection.QueryFirstOrDefaultAsync<string>(sql, new { EmployeeID = employeeID });
        }

        /// <summary>
        /// Cập nhật RoleNames cho nhân viên (lưu trực tiếp vào cột RoleNames trong bảng Employees).
        /// </summary>
        public static async Task<bool> UpdateEmployeeRoleAsync(int employeeID, string roleNames)
        {
            using var connection = new SqlConnection(Configuration.ConnectionString);
            const string sql = @"UPDATE Employees SET RoleNames = @RoleNames WHERE EmployeeID = @EmployeeID";
            int rows = await connection.ExecuteAsync(sql, new { RoleNames = roleNames ?? string.Empty, EmployeeID = employeeID });
            return rows > 0;
        }
